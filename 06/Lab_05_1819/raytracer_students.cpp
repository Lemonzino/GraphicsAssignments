...
Vec3f RayTracer::TraceRay (Ray & ray, Hit & hit, int bounce_count) const
{
  ...
  if (intersect == true)
  {
	...
	//T è il parametro al quale è avvenuta l'intersezione
	Vec3f point = ray.pointAtParameter (hit.getT ());

	// ----------------------------------------------
	// ambient light
	//Visto che ho colpito qualcosa aggiungo la luce ambiente
	...
	if (reflectiveColor.Length() > 0 && bounce_count > 0) {
		...
		answer += TraceRay(*newRay, hit, bounce_count--) * reflectiveColor;
	...
	// n è la normale | l è la direct to light
	// Sarebbe 2 * <n,l> * n - l poichè andiamo dall'oggetto  alla sorgente, su opengl invece dobbiamo andare dalla sorgente all'oggetto quindi abbiamo il contrario
	//     calcolare ReflectionRay  R= l - 2 * <n,l> * n

    //	   invocare TraceRay(ReflectionRay, hit,bounce_count-1)
	
	//Bisogna moltiplicare per kr -> Contributo luminoso
	//     aggiungere ad answer il contributo riflesso * kr
	...
	for (int i = 0; i < num_lights; i++)
	{
	  ...
	  //Direzione L -> dal punto al punto luce
