...
//Parametri di configurazione dell'onda
WaveParam p;
float t = 0.1f;

void wave_loop(int value) {
...
void init_textured_plane() {
	...
	//TODO: il texture only shader è da completare aggiungendo l'effetto di luce (shading oltre che solo texture)
	...
	//repeat perchè la texture deve essere ripetuta
...
void init_cube() {
	...
	//Shader associato
	obj1.shading = ShadingType::TEXTURE_PHONG;
...
void init_torus() {
	...
	//obj2.shading = ShadingType::TEXTURE_PHONG;
	//obj2.shading = ShadingType::TEXTURE_ONLY;
	//obj2.shading = ShadingType::PHONG; // PHONG
	//obj2.shading = ShadingType::BLINN; // BLINN
	//obj2.shading = ShadingType::GOURAUD; // GOURAUD
	obj2.shading = ShadingType::TOON; // PHONG-TOON
	...
	for (i = 0; i < 64; i++) {
		for (j = 0; j < 64; j++) {
			image[i][j][0] = (GLubyte)(glm::perlin(glm::vec3(i / 64.0, j / 64.0, (double)rand() / (RAND_MAX))) * 255);
			image[i][j][1] = (GLubyte)(glm::perlin(glm::vec3(i / 64.0, j / 64.0, (double)rand() / (RAND_MAX))) * 255);
			image[i][j][2] = (GLubyte)(glm::perlin(glm::vec3(i / 64.0, j / 64.0, (double)rand() / (RAND_MAX))) * 255);
		}
	}
	

	//UTILIZZO TEXTURE GENERATA PROCEDURALMENTE
	glGenTextures(1, &obj2.textureID);
...
void init_grid_plane() {
	Mesh _grid = {};
	loadObjFile(MeshDir + "GridPlane.obj", &_grid);
	generate_and_load_buffers(true, &_grid);
	Object obj1 = {};
	obj1.mesh = _grid;
	obj1.material = MaterialType::NO_MATERIAL;
...
// Default render settings
	...
	//Passiamo le variabili allo shader
	...
	//Toon Shader loading
	shaders_IDs[TOON] = initShader(ShaderDir + "v_toon_v2.glsl", ShaderDir + "f_toon_v2.glsl");
	...
	//Texture Shader loading
	shaders_IDs[WAVE] = initShader(ShaderDir + "v_wave.glsl", ShaderDir + "f_passthrough.glsl");
	//Otteniamo i puntatori alle variabili uniform per poterle utilizzare in seguito
	...
	p.a = glGetUniformLocation(shaders_IDs[WAVE], "a");
	p.w = glGetUniformLocation(shaders_IDs[WAVE], "w");
	p.t = glGetUniformLocation(shaders_IDs[WAVE], "t");
	glUseProgram(shaders_IDs[WAVE]);
	glUniform4fv(glGetUniformLocation(shaders_IDs[WAVE], "Color"), 1, value_ptr(glm::vec4(0, 1.0, 1.0, 1.0)));
	glUniform1f(p.a, 0.1f);
	glUniform1f(p.w, 0.001f);
	glUniform1f(p.t, t);

	//Pass-Through Shader loading -> Lo shader non fa nulla (non hanno illuminazione e texture)
...
void mouseClick(int button, int state, int x, int y)
{
	...
	case TRASLATING:
		...
		if (objects[selected_obj].name == "light") {
			glm::vec4 result = objects[selected_obj].M * glm::vec4(light.position, 1.0);
...
void material_menu_function(int option) 
{
	...
	case(MaterialType::RED_PLASTIC):
		objects[selected_obj].material = MaterialType::RED_PLASTIC;
		break;
	case(MaterialType::EMERALD):
		objects[selected_obj].material = MaterialType::EMERALD;
		break;
	case(MaterialType::BRASS):
		objects[selected_obj].material = MaterialType::BRASS;
		break;
	case(MaterialType::SLATE):
		objects[selected_obj].material = MaterialType::SLATE;
		break;
	}
}
...
void loadObjFile(string file_path, Mesh* mesh)
{
	...
	while (fscanf(file, "%s", lineHeader) != EOF) {
		if (strcmp(lineHeader, "v") == 0) {
			...
			GLuint t_a, t_b, t_c; // index in UV array -> texure
...
void computeTorusVertex(int i, int j, Mesh* mesh) {
	...
	//Calcolo del toro presente nel pdf dell'esercitazione
	//Calcolo i vertici e le normali
	...
	mesh->texCoords.push_back(glm::vec2( (1.0f/TorusSetup.NumWraps)*j, (1.0/TorusSetup.NumPerWrap)*i ));
}
